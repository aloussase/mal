...
find !env !s = fromMaybe (throw $ UnboundSymbol s) (go (scopeParent env) (scopeBindings env))
    where
        go (Just !scope') !bindings' = bindings' !? s <|> go (scopeParent scope') (scopeBindings scope')
